// =============================================================================
// /[unitSlug]/sonho/conquistado — Marcar sonho como conquistado
//
// SEGURANÇA:
//   • lead_id resolvido do cookie — nunca do browser
//   • unit_slug validado contra o banco
//   • allow_public_case só pode ser true com testimonial (validado no action)
//   • Soft delete via status = 'achieved' — nunca apaga fisicamente
// =============================================================================
...
import { markDreamAchieved } from '@/lib/actions'
import { ConquistadoForm } from './ConquistadoForm'

interface Props {
  params: Promise<{ unitSlug: string }>
}

export default async function ConquistadoPage({ params }: Props) {
  ...
  const { data: dream } = await supabase
    .from('dreams')
    .select('id, dream_type, dream_subtype, target_amount, target_label')
    .eq('lead_id', lead!.id)
  ...
  const boundAction = markDreamAchieved.bind(null, unitSlug)
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: C.bgApp, minHeight: '100dvh' }}>
      ...
      <main style={{ maxWidth: 480, margin: '0 auto', padding: '16px 16px 90px' }}>
        <ConquistadoForm dream={dream!} unitSlug={unitSlug} markAchievedAction={boundAction} />
      </main>
